...
const ScheduleManagement: React.FC = () => {
  ...
  const handleAddAppointment = (e: React.FormEvent) => {
    ...
    const financialRecord: FinancialRecord = {
      id: `f-${Date.now()}`,
      description: `Agendamento Manual (${app.type}) - ${
        client?.name || 'Cliente'
      }`,
  ...
  const handleTypeChange = (type: any) => {
    ...
    setNewApp({
      ...newApp,
      type,
      price: prices[type] || settings.defaultPrice,
      duration: durations[type] || settings.defaultDuration,
    });
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Gestão de horários de Patrícia Transista.
          </p>
        </div>
        <button
      ...
      <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col md:flex-row items-center gap-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="font-bold text-slate-800">Padrões do Salão</h3>
            <p className="text-xs text-slate-400">Valores e tempos base.</p>
          </div>
        </div>
        <div className="flex-1 grid grid-cols-2 gap-4 w-full">
          <div className="pl-12 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-2xl flex items-center justify-between relative">
            ...
            <input
              ...
              className="w-full bg-transparent text-right font-black text-amber-700 outline-none"
            />
          </div>
          <div className="pl-12 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-2xl flex items-center justify-between relative">
            <DollarSign
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between text-slate-600 text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      {new Date(app.date + 'T12:00:00').toLocaleDateString(
                        'pt-BR',
                      )}
                    </div>
                    <div className="flex items-center gap-2">
            ...
            <form onSubmit={handleAddAppointment} className="p-8 space-y-6">
              ...
              <div className="space-y-4 pt-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {['Box Braids', 'Nagô', 'Twist', 'Entrelace', 'Penteado'].map(
                    (t) => (
                      <button
                        type="button"
                        key={t}
              ...
              <button
                type="submit"
                className="w-full bg-amber-600 text-white py-4 rounded-xl font-bold text-lg hover:bg-amber-700"
              >
                Salvar Agendamento
              </button>
            </form>
