...
const KanbanBoard: React.FC<KanbanProps> = ({ tasks, setTasks }) => {
  ...
  return (
    <div className="space-y-6 lg:h-full lg:flex lg:flex-col animate-fade-in">
      <div className="flex justify-between items-center shrink-0">
        <div>
          <h2 className="text-2xl font-bold text-slate-800 tracking-tight">Gestão de Tarefas</h2>
          <p className="text-sm text-slate-500 font-medium">Organize processos e lembretes de Patrícia Transista.</p>
        </div>
      </div>

      <div className="lg:flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 lg:overflow-hidden pb-4">
        {columns.map(col => (
          <div key={col.id} className="bg-slate-100/50 rounded-[2rem] border border-slate-200 flex flex-col min-h-[400px] lg:h-full shadow-sm overflow-hidden">
            <div className="p-5 border-b border-slate-200 flex justify-between items-center shrink-0 bg-white/40">
              ...
              <button 
                ...
                className="p-1.5 hover:bg-white rounded-xl text-slate-500 hover:text-amber-600 transition-all shadow-sm border border-transparent hover:border-slate-200"
            ...
            <div className="flex-1 p-4 space-y-4 lg:overflow-y-auto custom-scrollbar">
              {tasks.filter(t => t.status === col.id).map(task => (
                <div 
                  key={task.id} 
                  className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 group transition-all hover:shadow-xl hover:border-amber-100 relative overflow-hidden animate-scale-up"
                >
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <button 
                      ...
                      className="opacity-100 lg:opacity-0 group-hover:opacity-100 text-slate-300 hover:text-red-500 p-1.5 transition-all rounded-lg hover:bg-red-50"
                    >
                      <Trash2 size={14} />
                    </button>
                  </div>
