...
const KanbanBoard: React.FC = () => {
  const { kanbanTasks: tasks, setKanbanTasks: setTasks } = useAppContext();
  const columns: {
    ...
    <div className="space-y-6 lg:h-full lg:flex lg:flex-col animate-fade-in">
      <div className="flex justify-between items-center shrink-0">
        <div>
          ...
          <p className="text-sm text-slate-500 font-medium">
            Organize processos e lembretes de Patrícia Transista.
          </p>
        </div>
      </div>
      ...
      <div className="lg:flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 lg:overflow-hidden pb-4">
        {columns.map((col) => (
          <div
            key={col.id}
            className="bg-slate-100/50 rounded-[2rem] border border-slate-200 flex flex-col min-h-[400px] lg:h-full shadow-sm overflow-hidden"
          >
